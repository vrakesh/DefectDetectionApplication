...
class PretrainedFeatureGaussianPostProcessor(InferencePostProcessor):
    ...
    def __call__(self, model_output: List[np.ndarray], *args, output_features=False, **kwargs):
        ...
        # TODO: for now we suppress returning the mask until, we are ready to consume it downstream
        # masks = convert_image_to_numpy(masks[0])
        masks = None
        if output_features:
